...
-(UIColor *)attributeTextColor {
    ...
    return _attributeTextColor;
}

-(void)setAttributeTextColor:(UIColor *)attributeTextColor {
    if(attributeTextColor != _attributeTextColor) {
        _attributeTextColor = attributeTextColor;
        [self setNeedsDisplay];
    }
}

-(UIColor *)horizontalLineColorRadial {
    if(!_horizontalLineColorRadial) {
        _horizontalLineColorRadial = [UIColor blackColor];
    }
    return _horizontalLineColorRadial;
}

-(void)setHorizontalLineColorRadial:(UIColor *)horizontalLineColorRadial {
    if(horizontalLineColorRadial != _horizontalLineColorRadial) {
        _horizontalLineColorRadial = horizontalLineColorRadial;
        [self setNeedsDisplay];
    }
}
...
- (void)setColors:(NSArray *)colors {
    ...
    for (UIColor *color in colors) {
        [self.legendView.colors addObject:color];
    }
}
...
- (void)drawRect:(CGRect)rect {
    NSArray *strokeColors = [self.legendView.colors copy];
    NSArray *fillColors = [[self.legendView colorsWithOpacity:self.colorOpacity] copy];
    ...
    [self.horizontalLineColorRadial setStroke];
    ...
    if (_numOfV > 0) {
        for (int serie = 0; serie < [_dataSeries count]; serie++) {
            if (self.fillArea) {
                [fillColors[serie] setFill];
            }
            else {
                [strokeColors[serie] setStroke];
            }
            for (int i = 0; i < _numOfV; ++i) {
            ...
            if (_drawPoints) {
                for (int i = 0; i < _numOfV; i++) {
                    ...
                    CGContextStrokeEllipseInRect(context, CGRectMake(xVal - 4, yVal - 4, 8, 8));
